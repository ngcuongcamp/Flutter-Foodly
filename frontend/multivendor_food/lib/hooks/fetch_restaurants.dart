...
FetchHook useFetchRestaurants(String code) {
  // final restaurantsItems = useState<List<RestaurantsModel>?>(null);
  final restaurantsItems = useState<List<RestaurantsModel>?>([]);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (jsonResponse['status'] == true && jsonResponse['data'] is List) {
          restaurantsItems.value = List<RestaurantsModel>.from(
